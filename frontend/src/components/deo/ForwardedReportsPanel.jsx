...
export default function ForwardedReportsPanel({ district, className = "" }) {
  ...
  return (
    <Card
      variant="gov"
      className={className}
      title="Forwarded School Reports"
      subtitle="Bundles forwarded by principals · ranked by LR urgency"
      icon={FileText}
    >
      ...
      {loading ? (
        <div className="py-12 text-center text-slate-400 text-sm">Loading forwarded bundles…</div>
      ) : bundles.length === 0 ? (
        <div className="py-12 text-center text-slate-400 font-medium text-sm italic bg-slate-50/50 rounded border border-dashed border-slate-200">
          No reports have been forwarded by principals yet.
        </div>
      ) : (
        <div className="space-y-3">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded ${u.dot} text-white flex flex-col items-center justify-center font-black`}>
                    <span className="text-[8px] uppercase opacity-70 leading-none">W</span>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {b.willFailWithin30Days && (
                        <Badge variant="critical" size="sm">
                          <AlertTriangle size={10} className="mr-1" /> 30-DAY FAIL
                        </Badge>
                      )}
                    </div>
                    <p className="text-[11px] text-slate-500 font-medium mt-0.5">
                      {b.district} · Worst: {b.worstCategory} · Forwarded {b.forwardedAt ? new Date(b.forwardedAt).toLocaleString() : "—"}
                    </p>
                  </div>
                </div>
          ...
          {bundles.length > 5 && (
            ...
            >
              + {bundles.length - 5} more bundles · View all
            </button>
          )}
        </div>
      )}
